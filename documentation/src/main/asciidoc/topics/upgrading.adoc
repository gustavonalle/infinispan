...
* Memcached was deprecated in 10.1 and now it is removed.
...
* The ```index()``` (index mode) configuration is deprecated. Infinispan will automatically figure out how to index the entries. The following substitutions should be made in the configuration:
    ** `.indexing().index(Index.PRIMARY_OWNER)` => `index.enabled(true)`
    ** `.indexing().index(Index.ALL)` => `index.enabled(true)`
    ** `.indexing().index(Index.NONE)` => `index.enabled(false)`
