...
== REST and Hot Rod Interoperability with Text-Based Storage
...
== REST and Hot Rod Interoperability with Custom Java Objects
...
[source,xml,options="nowrap",subs=attributes+]
----
include::config_examples/encoding_jboss_marshalling.xml[]
----
...
. Stop {brandname} if it is running. {brandname} only loads entity classes at
boot time.
. Copy the JAR to the `server/lib` directory of your {brandname} server installation.
+
----
├── server
│   ├── lib
