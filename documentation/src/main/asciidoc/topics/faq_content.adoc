...
{brandname} offers support for an optimised {brandname}-specific protocol called Hot Rod.
...
If passivation is disabled, the cache store behaves as a write-through (or write-behind if asynchronous) cache, where all entries in memory are also maintained in the cache store.
...
[source,java]
----
include::code_examples/GetAdvancedCacheWithFlagsSkipCacheLoad.java[]
----
...
[source,java]
----
include::code_examples/LockingModePessimistic.java[]
----
...
[source,java]
----
include::code_examples/LockingModePessimisticImplicitAcquire.java[]
----
...
[source,java]
----
include::code_examples/LockingModePessimisticExplicitAcquire.java[]
----

==== What isolation levels does {brandname} support?
{brandname} only supports the isolation levels *READ_COMMITTED* and *REPEATABLE_READ*.
...
| _API_ | Usage of Asynchronous API, i.e. methods of the Cache interface like e.g. putAsync(key, val)
...
[source,java]
----
include::code_examples/GetClassLoader.java[]
----
...
[source,java]
----
include::code_examples/ABCMarshallingExternalizerWrong.java[]
----
...
[source,java]
----
include::code_examples/ABCMarshallingExternalizer.java[]
----
...
----
ERROR [org.infinispan.remoting.transport.jgroups.JGroupsTransport] (STREAMING_STATE_TRANSFER-sender-1,{brandname}-Cluster,NodeJ-2368:) Caught while responding to state transfer request
...
Since version 4.1, whenever {brandname} needs to print byte arrays to logs, these are partially printed in order to avoid unnecessarily printing potentially big byte arrays. This happens in situations where either, {brandname} caches have been configured with lazy deserialization, or Hot Rod server.
