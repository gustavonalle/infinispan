...
public class CollectionsIndexingTest extends SingleCacheManagerTest {
   ...
   private CacheQuery<?> getCountryQuery(Class<?>... clazz) {
      String q = String.format("FROM %s where countryName:'Italy'", Country.class.getName());
      return searchManager.getQuery(q, FETCH, clazz);
   }
   ...
   public void searchOnAllTypes() {
      ...
      cache.put("IT", italy);
      List<?> list = getCountryQuery(Country.class, City.class).list();
   ...
   public void searchOnEmbeddedField() {
      ...
      List<?> list = searchManager.getQuery(q, FETCH, Country.class, City.class).list();
      assertEquals(1, list.size());
