...
public class MultipleEntitiesTest extends SingleCacheManagerTest {
   ...
   public void testIndexAndQuery() {
      ...
      CacheQuery<?> query = searchManager.getQuery("FROM " + Bond.class.getName(), FETCH, Bond.class, Debenture.class);
      assertEquals(query.list().size(), 3);

      CacheQuery<?> queryBond = searchManager.getQuery("FROM " + Bond.class.getName(), FETCH, Bond.class);
      assertEquals(queryBond.getResultSize(), 2);

      CacheQuery<?> queryDeb = searchManager.getQuery("FROM " + Debenture.class.getName(), FETCH, Debenture.class);
