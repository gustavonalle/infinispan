...
public class IndexStoredIndexedCacheTest extends MultipleCacheManagersTest {
   ...
   public void testIndexWritingAndFinding() throws IOException {
      ...
      Directory dirA = DirectoryBuilder.newDirectoryInstance(cache0, cache0, cache0, indexName).overrideSegmentReadLocker(readLocker).create();
      Directory dirB = DirectoryBuilder.newDirectoryInstance(cache1, cache1, cache1, indexName).overrideSegmentReadLocker(readLocker).create();
