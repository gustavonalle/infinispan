...
public class ReadAfterLostDataTest extends MultipleCacheManagersTest {
   ...
   //TODO: We don't test put/remove/replace/readWrite before topology update as with triangle these commands
   ...
   protected void test(BiFunction<Cache<Object, Object>, Collection<?>, Map<?, ?>> operation, boolean write, boolean blockUpdates) throws Exception {
      ...
      keys.add(new MagicKey(cache(0), cache(1))); // both owners in p0
      keys.add(new MagicKey(cache(0), cache(2))); // primary in p0
      keys.add(new MagicKey(cache(2), cache(1))); // backup in p0
      keys.add(new MagicKey(cache(2), cache(3))); // nothing in p0
   ...
   private static Map<?, ?> putMap(Cache<Object, Object> cache, Collection<?> keys) {
      ...
      Map<?, ?> previousValues = (Map<?, ?>) cache.getAdvancedCache().getAsyncInterceptorChain().invoke(ctx, putMapCommand);
